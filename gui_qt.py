...
try:
    ...
    logging.warning(f"Core imports failed in gui_qt.py: {e}")
...
class EmissionGUI:
    ...
    def _merged(self, plot_by_mode=None, scc_selection=None, scc_code_map=None, notify=None, pollutant=None, fill_nan=False) -> Optional[gpd.GeoDataFrame]:
        """Prepare and merge emissions with geometry, mirroring gui.py logic."""
        ...
        if target_pol and hasattr(self.emissions_df, 'columns') and target_pol not in self.emissions_df.columns:
            ...
            if ds is not None:
                try:
                    ...
                    path = self.inputfile_path if getattr(self, 'inputfile_path', None) else None
                    if path:
                        # Extract first path if multiple
        ...
        if mode == 'grid':
            ...
            if self.counties_gdf is None:
                if self.grid_gdf is not None:
                    _do_notify('INFO', 'Geo Fallback', 'Counties loading... Using Grid view.')
                    base_gdf = self.grid_gdf
                    merge_on = 'GRID_RC'
                    geometry_tag = 'grid'
                    if not is_native: self._ensure_ff10_grid_mapping(notify_success=False)
            ...
            if base_gdf is None and self.counties_gdf is not None:
                ...
                geometry_tag = 'county'
            if base_gdf is None:
                _do_notify('WARNING', 'No geometry', 'Could not find a suitable shapefile.')
                raise ValueError("Handled")

        if merge_on and isinstance(base_gdf, gpd.GeoDataFrame):
            if merge_on not in base_gdf.columns:
                if merge_on == 'FIPS':
                    col = next((c for c in ['GEOID', 'region_cd'] if c in base_gdf.columns), None)
                    if col:
        ...
        try:
            ...
            if filter_mode != 'False' and self.filter_gdf is not None:
                try: merged = apply_spatial_filter(merged, self.filter_gdf, filter_mode)
                except: pass
    ...
    def _handle_cell_click(self, x, y):
        ...
        try:
             ...
             # Check if ncf (plot_controls equivalent)
             is_ncf = False
             try:
                 for p_win in self._plot_windows.values():
                     if p_win.get('win') and p_win['win'].isVisible():
                         # In this shimmed Qt version, we check if it's a NetCDF source
                         is_ncf = p_win.get('is_ncf', False)
                         break
             except: pass
             ...
             # Layer/Op (simplified for now, defaults to Layer 1)
             l_idx = 0; l_op = 'select'
    ...
    def _show_ts_window(self, data, title):
        try:
             ...
             win = TimeSeriesPlotWindow(data, title, pol, unit, self.root)
             win.show()
        except Exception as e:
            logging.error(f"TS Window error: {e}")

class TimeSeriesPlotWindow(QtWidgets.QDialog):
